...
import { mapLogger as _mapLogger } from '../../utils/logger';
        ...
        >
          ...
          >
            ...
            {realCaliforniaCountyShapes.map((county) => {
              ...
              const isFiltered =
                filteredCounties && !showAllCounties
              ...
              return (
                <g key={county.id}>
                  <path
                    d={county.path}
                    fill={fillColor}
                    fillOpacity={!isFiltered ? 0.3 : isSelected ? 0.98 : isHovered ? 0.9 : 0.7}
                    stroke={
                      !isFiltered
                        ? '#D1D5DB'
                        : isSelected
                    ...
                    strokeWidth={!isFiltered ? 0.5 : isSelected ? 3.5 : isHovered ? 2.5 : 1}
                    strokeLinejoin="round"
                    strokeLinecap="round"
                    data-clickable={isFiltered ? 'true' : 'false'}
                    className={
                      isFiltered
                        ? 'cursor-pointer transition-all duration-200 hover:filter hover:brightness-110'
          ...
          <g transform="translate(650, 20)">
            ...
            {Object.entries(REGION_COLORS).map(([regionName, config], idx) => (
              <g key={regionName} transform={`translate(0, ${20 + idx * 15})`}>
                ...
                <text x="15" y="8" className="text-xs fill-gray-600">
                  {regionName.replace('California', 'CA').replace('Central ', '')}
                </text>
              </g>
            ))}
          </g>
        ...
        {hoveredCounty && (
          <div
            ...
            style={{
              ...
              left:
                mousePosition.x > window.innerWidth - 250
            ...
            <div className="flex items-center gap-3">
              <div>
                <div className="text-sm font-bold text-gray-900">
                  {getCountyInfo(hoveredCounty)?.name}
                </div>
                <div className="text-xs text-gray-600 mt-0.5">
                  {getCountyInfo(hoveredCounty)?.region}
                </div>
              </div>
              ...
              <svg
                ...
                height="32"
                viewBox="0 0 800 900"
                className="border border-gray-200 rounded bg-gray-50 flex-shrink-0"
              >
                <path
