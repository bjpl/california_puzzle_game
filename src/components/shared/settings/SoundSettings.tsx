...
export const SoundSettings: React.FC<SoundSettingsProps> = ({ className, onClose }) => {
  ...
  const handleVolumeChange = (
    type: 'masterVolume' | 'effectsVolume' | 'musicVolume',
    value: number
  ) => {
    ...
    <div
      className={`flex items-start gap-3 p-3 rounded-lg border ${
        disabled
          ...
          : 'bg-white border-gray-200 hover:border-ca-ocean-300'
      } transition-colors`}
    >
      <div className="flex-shrink-0 mt-0.5 text-ca-ocean-600">{icon}</div>
      <div className="flex-1">
        <div className="flex items-center justify-between">
          <h4 className={`font-medium ${disabled ? 'text-gray-400' : 'text-gray-900'}`}>{label}</h4>
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div
              className={`w-11 h-6 rounded-full transition-colors ${
                disabled ? 'bg-gray-200' : checked ? 'bg-ca-tech-500' : 'bg-gray-300'
              }`}
            >
              <div
  ...
  return (
    <div className={`bg-white rounded-xl shadow-lg p-6 max-w-md w-full ${className}`}>
      <div className="flex items-center justify-between mb-6">
        ...
        {onClose && (
          <CaliforniaButton variant="ghost" size="sm" onClick={onClose} enableSounds={false}>
            ✕
          </CaliforniaButton>
        )}
      </div>
      ...
      <div className="mt-6 p-3 bg-gray-50 rounded-lg">
        ...
        <p className="text-xs text-gray-600">
          The game includes placeholder tones that work without external sound files. For enhanced
          audio experience, place MP3 files in the{' '}
          <code className="bg-gray-200 px-1 rounded">public/sounds/</code> directory.
        </p>
      </div>
