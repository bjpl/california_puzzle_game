...
import { getUnlockedModes, getModeById } from '@/config/gameModes';
...
export const ProgressionSystem: React.FC<ProgressionSystemProps> = ({
  ...
  const modeUnlockStatuses = useMemo((): ModeUnlockStatus[] => {
    ...
    return availableModes.map((mode) => {
      ...
      const unlockProgress: UnlockProgress[] =
        mode.unlockRequirements?.map((req) => {
  ...
  const progressionStats = useMemo(() => {
    ...
    const nextUnlockDistance =
      lockedModes.length > 0
        ? Math.min(
            ...lockedModes.map((s) =>
              s.unlockProgress.reduce(
                (total, p) => total + Math.max(0, p.targetProgress - p.currentProgress),
                0
              )
            )
          )
    ...
    <div key={index} className="flex items-center gap-3 p-2 bg-gray-50 rounded">
      <div
        className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
          progress.isCompleted ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300'
        }`}
      >
      ...
      <div className="flex-1">
        <div className="text-sm font-medium text-gray-900">{progress.description}</div>
    ...
    return (
      ...
      >
        ...
        <div className="space-y-2">
          {status.unlockProgress.map((progress, index) => renderUnlockRequirement(progress, index))}
        </div>
      ...
      {progressionStats.nextUnlockDistance > 0 && (
        <div className="mt-4 p-3 bg-white bg-opacity-60 rounded">
          <div className="text-sm text-gray-700">
            <span className="font-medium">Next unlock:</span> Complete{' '}
            {progressionStats.nextUnlockDistance} more requirement(s)
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className={className}>
      ...
      <div className="space-y-4">
        ...
        {modeUnlockStatuses.filter((status) => !status.isUnlocked).map(renderModeUnlockCard)}

        {modeUnlockStatuses.every((status) => status.isUnlocked) && (
          <div className="text-center py-8">
            <div className="text-4xl mb-2">🏆</div>
            <h3 className="text-xl font-bold text-gray-900 mb-2">All Modes Unlocked!</h3>
