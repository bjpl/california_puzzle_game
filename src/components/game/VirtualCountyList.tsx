...
const CountyRow = memo<RowProps>(({ index, style, data }) => {
  ...
  const handleClick = useCallback(
    ...
    [isPlaced, isSelected, sound, data.onCountySelect, county]
  );
  ...
  return (
    ...
    prevProps.data.placedCounties.has(prevCounty.id) === nextProps.data.placedCounties.has(nextCounty.id) &&
...
export const VirtualCountyList: React.FC<VirtualCountyListProps> = memo(({
  ...
  const organizedCounties = useMemo(() => {
    ...
    // Group by region
    const grouped = counties.reduce((acc, county) => {
