...
export const useDragAndDrop = (
    ...
    (position: Position): Position => {
      ...
      if (snapToGrid?.enabled && snapToGrid.size > 0) {
        constrainedPosition.x =
          Math.round(constrainedPosition.x / snapToGrid.size) * snapToGrid.size;
        constrainedPosition.y =
          Math.round(constrainedPosition.y / snapToGrid.size) * snapToGrid.size;
      }
  ...
  const handlePointerDown = useCallback(
    (event: React.PointerEvent, item: CountyPiece) => {
