...
import {
  getCountyById,
  californiaCounties as _californiaCounties,
} from '@/data/californiaCounties';
...
export function analyzeStruggle(struggle: StruggleData): HintType[] {
  ...
  if (
    struggle.wrongPlacements.length >= 2 &&
    !struggle.suggestedHints.includes(HintType.NEIGHBOR)
  ) {
    const avgX =
      struggle.wrongPlacements.reduce((sum, pos) => sum + pos.x, 0) /
      struggle.wrongPlacements.length;
    const avgY =
      struggle.wrongPlacements.reduce((sum, pos) => sum + pos.y, 0) /
      struggle.wrongPlacements.length;

    // If placements are scattered, suggest neighbor hint
    const variance =
      struggle.wrongPlacements.reduce(
        (sum, pos) => sum + Math.pow(pos.x - avgX, 2) + Math.pow(pos.y - avgY, 2),
        0
      ) / struggle.wrongPlacements.length;

    if (variance > 10000) {
      // High variance indicates scattered attempts
...
function generateNameHint(county: County, _progress: number): string {
