...
```tsx
<h3 className="text-xl font-bold text-ca-charcoal-700...">{county.name}</h3>
```
...
```typescript
const [touchTargetSize, setTouchTargetSizeState] = useState<TouchTargetSize>(getTouchTargetSize());
```
...
<nav aria-label="Game phase">
  <ol className="flex gap-2 text-sm">
    <li className={gamePhase === 'mode_selection' ? 'font-bold' : 'text-gray-500'}>Select Mode</li>
    <li aria-hidden="true">→</li>
    <li className={gamePhase === 'playing' ? 'font-bold' : 'text-gray-500'}>Playing</li>
  </ol>
</nav>
```
...
```tsx
-'bg-gray-100 text-gray-400 cursor-not-allowed' + 'bg-gray-100 text-gray-500 cursor-not-allowed';
```
